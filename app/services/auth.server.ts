...
const getEmailFromGithub = async (tokens: OAuth2Tokens) : Promise<string | null> => {
    ...
    if (data && data.length > 0) {
        return data.find((email: { primary: boolean, verified: boolean, email: string }) => email.primary && email.verified)?.email;
    }
